...
import { Box, Package, Clock, BarChart, Search, MoreHorizontal, ChevronRight, Home, Users, Settings, Menu, X, AlertTriangle, Euro, Store } from 'lucide-react'
...
export function StockDashboard() {
  ...
  const productsOnMarketplace = 178 // Nombre de produits sur la marketplace

  return (
    <div className="flex min-h-screen bg-gray-50/50">
      {/* Sidenav */}
      <aside className={`fixed md:static w-[240px] h-screen bg-white border-r border-gray-100 p-5 transition-all duration-300 z-50 
        ${isSidenavOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="flex items-center gap-3 mb-6">
          <div className="w-8 h-8 rounded-xl bg-blue-500 flex items-center justify-center">
            <Package className="h-5 w-5 text-white" />
          </div>
          <span className="text-lg font-semibold">Cousit Marketplace</span>
        </div>

        <nav className="space-y-1">
          <button className="flex items-center w-full gap-3 p-2.5 text-blue-600 bg-blue-50 rounded-lg text-sm">
            <Home className="h-4 w-4" />
            <span className="font-medium">Tableau de bord</span>
          </button>
          <button 
            onClick={() => navigate('/products')}
            className="flex items-center w-full gap-3 p-2.5 text-gray-600 hover:bg-gray-50 rounded-lg text-sm"
          >
            <Package className="h-4 w-4" />
            <span>Produits</span>
          </button>
          <button className="flex items-center w-full gap-3 p-2.5 text-gray-600 hover:bg-gray-50 rounded-lg text-sm">
